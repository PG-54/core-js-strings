...
function countVowels(str) {
  return [...str].reduce((a, b) => a + ('aeiouyAEIOUY'.includes(b) ? 1 : 0), 0);
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduceRight((a, b) => (a.length > b.length ? a : b), '');
}
...
function extractNameFromTemplate(value) {
  return value.slice(7).slice(0, value.slice(7).length - 1);
}
...
function unbracketTag(str) {
  return str.slice(1).slice(0, str.slice(1).length - 1);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (!letters.includes(char)) {
      ...
      char = String.fromCharCode(-(13 - char.charCodeAt(0)));
      res = res.concat(char);
    } else {
